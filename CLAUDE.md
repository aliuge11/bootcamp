...
- PowerPoint'e gömme için üçüncü parti "Web Viewer" eklentilerine (çoğu ücretli veya kaldırılmış) bağımlı kalmamak için `public/addin/` altında kendi PowerPoint content add-in'imiz var. İki manifest: `manifest.xml` (klasik "Eklentimi Yükle" penceresi bunu ister — çoğu masaüstü sürümde geçerli olan) ve `manifest.json` (yeni unified manifest). Her ikisi de `SourceLocation`/`code.page` olarak `public/addin/index.html` sarmalayıcısını gösteriyor: bu sayfa office.js yükleyip (Office.onReady sinyali için) uygulamayı tam boyut iframe'liyor. Sideload: PowerPoint → Ekle → Eklentiler → EKLENTİLERİM → Eklentimi Yükle → `manifest.xml`.
