...
- **2026-07-19 — PLAN.md Adım 1:** Yürütmeye başlandı. Makinede Node.js ve Docker hiç kurulu değildi — kullanıcı onayıyla winget ile Node.js LTS ve (Docker yerine, EDB'nin indirme CDN'i bu ortamdan 403 verdiği için — bkz. Bilinen Gerçekler) native PostgreSQL 16 kuruldu. Next.js iskeleti (App Router, TS), Tailwind v4 (DESIGN.md token'larından `@theme`), `frame-ancestors *` CSP başlığı kuruldu; `react-simple-maps` React 19 peer-conflict nedeniyle terk edilip doğrudan `d3-geo`+`topojson-client` kullanılacak şekilde CLAUDE.md/PLAN.md güncellendi. Build ve dev server tarayıcıda doğrulandı (doğru renk/font/header).
