...
- **Compact:** Kargo / İçi / Dışı, `grid-cols-3` ile yan yana, küçük tipografi (headline-sm, 18px) ve daha az padding (`compact` prop'u). Etiketler kısa tutuluyor ("SLA" öneki yok) — üstte hero başlığı ve rozet zaten "SLA" bağlamını kurmuş durumda, üç sütunun dar (~49px) genişliğinde "SLA İçi"/"SLA Dışı" gibi daha uzun etiketler 2 satıra kırılıyor ve o satırdaki kutuyu diğer ikisinden görünmez şekilde daha uzun bırakıyordu.
