...
"Bölge bazlı" seçiliyken ve hiçbir şehir/ilçe tıklanmamışken, detay panelinin göründüğü sütunda onun yerine bir "Bölge Karşılaştırması" listesi açılıyor — 7 bölgenin hepsi, SLA dışı oranına göre **en kötüden en iyiye** sıralı (operasyonel önceliğe göre: önce sorunlu bölgeler görünür). Her satırda bölge adı, toplam kargo sayısı ve o bölgenin SLA dışı oranını gösteren aynı `Badge` bileşeni (harita/detay panelindeki ile birebir aynı 5 renk). Bir bölgede hiç veri yoksa o bölge listeden düşmüyor — "veri yok" gri rozetle (haritadaki no-data kuralıyla tutarlı) 0 kargo ile listede kalıyor, böylece "bu bölge unutuldu mu" belirsizliği olmuyor. Bir ile tıklanıp ilçelere girildiğinde (veya İl bazlı'ya geçildiğinde) bu liste kayboluyor, normal il/ilçe detay paneli onun yerini alıyor.
