...
Bir ile girip ilçe haritasındayken, il bazlı haritaya dönmenin iki hızlı yolu daha var: haritada **boş bir alana** (ilçe olmayan yer/harita dışı siyah alan) tıklamak, veya **Esc** tuşu. İkisi de seçimi Türkiye görünümüne (`null`) döndürüyor — Geri butonuna gitmeden çıkış. (İlçe şekline tıklama `stopPropagation` ile boş-alan handler'ına ulaşmıyor, sadece o ilçeyi seçiyor.) Karşılaştırmada Esc tüm haritalarda aynı anda tetikleniyor ama `useSelectionHistory` aynı değeri arka arkaya itmediği için geçmiş bozulmuyor.

### İl/Bölge etiket toggle'ı
Türkiye genel görünümünde (bir ile girilmediğinde), haritanın üstünde iki buton: "İl bazlı" (varsayılan) ve "Bölge bazlı". Bu sadece bir **etiketleme** tercihi — dolgu rengi her zaman il bazlı SLA verisine göre hesaplanıyor, il sınırları hep çiziliyor, tıklama/drill-down davranışı değişmiyor. "Bölge bazlı" seçilince sadece 81 il adı yerine Türkiye'nin 7 coğrafi bölgesinin adı (Marmara, Ege, Akdeniz, İç Anadolu, Karadeniz, Doğu Anadolu, Güneydoğu Anadolu) gösteriliyor — üst düzey bir sunumda "hangi bölgedeyiz" hissi vermek için, verinin kendisini bozmadan. Bir ile tıklayıp ilçelere girildiğinde bu toggle kayboluyor (bölge kavramı ilçe seviyesinde anlamsız). Karşılaştırma görünümünde (2-4 harita) bu toggle **senkron** — herhangi bir haritada "Bölge bazlı"ya basmak hepsini aynı anda değiştiriyor, tıpkı şehir seçiminin (Geri/İleri) senkron olması gibi. Tek harita görünümünde her zamanki gibi bağımsız/yerel.

"Bölge bazlı"da ayrıca, farklı bölgelerdeki komşu iller arasındaki sınır ayrı bir çizgi türüyle vurgulanır: {colors.map-boundary}, 5px, **kesikli** (`9 5` dash deseni, yuvarlak uçlu). Sadece kalınlık farkı (ör. 3.5px vs 1.5px) aynı renkte iki çizgiyi uzaktan/projeksiyonda ayırt etmeyi zorlaştırıyordu — kesikli desen, kalınlıktan bağımsız olarak "bu farklı bir kademe sınırı" mesajını anında veriyor (kartografide idari üst-kademe sınırları için standart bir teknik). Bu, topojson'un paylaşılan ark (arc) yapısından `topojson-client`'ın `mesh()` fonksiyonuyla türetiliyor — poligonlar gerçekten birleştirilmiyor (bkz. Do's/Don'ts), sadece "iki tarafındaki il farklı bölgedeyse" filtresiyle o kesişim çizgileri ayrı bir path olarak üstüne çiziliyor. Aynı bölgedeki komşu iller arasında hâlâ sadece normal (ince, düz) il sınırı var.

### Bölge Karşılaştırması
"Bölge bazlı" seçiliyken ve hiçbir şehir/ilçe tıklanmamışken, detay panelinin göründüğü sütunda onun yerine bir "Bölge Karşılaştırması" listesi açılıyor — 7 bölgenin hepsi, SLA dışı oranına göre **en kötüden en iyiye** sıralı (operasyonel önceliğe göre: önce sorunlu bölgeler görünür). Her satırda bölge adı, iki sabit satırlık sayı bloğu ("{kargo} kargo · {SLA içi} SLA İçi" ve altında "{SLA dışı} SLA Dışı" — büyük sayılarda tutarsız kırılma olmasın diye kasıtlı olarak hep 2 satır) ve o bölgenin SLA dışı oranını gösteren aynı `Badge` bileşeni (harita/detay panelindeki ile birebir aynı 5 renk). Badge her zaman `shrink-0` + `whitespace-nowrap` — satırdaki metin bloğu büyüyünce badge'in sıkışıp yüzdenin 2 satıra kırılmasını önlüyor (bkz. Do's/Don'ts, StatTile'daki benzer taşma dersi). Bir bölgede hiç veri yoksa o bölge listeden düşmüyor — "veri yok" gri rozetle (haritadaki no-data kuralıyla tutarlı) 0 kargo ile listede kalıyor, böylece "bu bölge unutuldu mu" belirsizliği olmuyor. Bir ile tıklanıp ilçelere girildiğinde (veya İl bazlı'ya geçildiğinde) bu liste kayboluyor, normal il/ilçe detay paneli onun yerini alıyor.

### Karşılaştırma özeti
Tam olarak iki harita karşılaştırılırken (elle seçim veya ana sayfadaki "İlk ve son yüklemeyi karşılaştır" kısayolu), ızgaranın altında stat-tile'a benzer tek bir kart. Başlık, o an haritalarda seçili olan kapsamı gösteriyor: hiçbir şey seçili değilse "Türkiye (toplam)", bir il seçiliyse il adı, bir ilçe seçiliyse "İl / İlçe". Eski→yeni etiketi tarih değil, **haritanın kendi adı** (`display_name` veya yoksa dosya adı) — ör. "Mayıs → Haziran" — kullanıcı haritalarını anlamlı isimlerle yüklediğinde karşılaştırma cümlesi tarih yerine o ismi kullanıyor. İçerik her zaman somut sayı: başarı oranı eski→yeni ok işaretiyle, puan farkı ve yön ("arttı"/"azaldı"/"değişmedi"), kargo sayısı eski→yeni. Bir haritada veri yoksa (kargo 0), yüzde/puan hesaplanmıyor — "veri yok, karşılaştırma yapılamıyor" gösteriliyor, sessizce %0 gibi yanıltıcı bir sayı üretilmiyor. Bu kart üç veya dört haritalı karşılaştırmada gösterilmiyor (karşılaştırma çifte anlamlı, ikiden fazla haritada "eski/yeni" belirsizleşir).

Karşılaştırma ızgarasındaki harita sırası da her zaman **kronolojik**: en eski yüklenen harita solda/en üstte, en yeni sağda/en altta — kullanıcının haritaları hangi sırayla seçtiğinden (ör. geçmiş listesinde tıklama sırası) bağımsız. Bu, `/map/compare` sayfasında `maps` dizisi render edilmeden önce `uploaded_at`'e göre sıralanarak sağlanıyor; URL'deki `ids` parametresinin sırası önemli değil.
...
- **Etiketler her zaman en üstte (iki geçiş):** Harita önce tüm il/ilçe **dolguları**, sonra tüm **etiketler** olacak şekilde iki ayrı geçişte çiziliyor. Aksi halde SVG'de sonradan çizilen komşu ilin dolgusu, önceki ilin etiketinin üzerini örtüp "eksik/kaybolan etiket" sorununa yol açıyordu. Aynı sebeple **seçili şehir konturu** (primary) da dolguların üstünde, ayrı bir geçişte çiziliyor — komşu dolgu onu da örtmesin.
- **Kenar boşluğu:** Harita SVG'nin kenarına dayanmıyor; `fitExtent` ile ~40px yatay / ~24px dikey iç boşluk bırakılıyor. Böylece en doğu/batı/kuzey/güney illerin adları (ör. "Ardahan") SVG dışına taşıp `overflow: hidden` ile kırpılmıyor. Overflow kırpması hâlâ açık (uzun ilçe adları için güvenlik ağı); boşluk sadece normal etiketlerin sığmasını garantiliyor.
